...
/**
 * 매일 오전 10시·오후 5시 법원 뉴스 + 판결기사 크롤링 후 DB 동기화 및 보고 메일 발송
 */
@Injectable()
export class LawNewsDailyService {
  ...
  /**
   * 두 크롤러 실행 후 결과를 모아 이메일 발송 (크론·수동 공통)
   */
  async runDailyJob(): Promise<void> {
    ...
    this.logger.log(
      `[LawNewsDaily] 크론 실행 — 법원 뉴스·판결기사 수집 및 메일 발송 시작 (${runAt})`,
    );

    const [legalStats, judgmentStats] = await Promise.all([
    ...
    if (sent) {
      this.logger.log(
        `[LawNewsDaily] 보고 메일 발송 완료 — 법원 ${legalStats.created}건 신규, 판결 ${judgmentStats.created}건 신규`,
      );
    } else {
